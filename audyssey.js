const CHANNEL_TYPES = {
  ...
  getByMeasurementOrder(order) {
    return Object.entries(this).find(([key, value]) =>
      value.measurementOrder === order && typeof value === 'object'
    )?.[1];
  },

  getByChannelIndex(index) {
    return Object.entries(this).find(([key, value]) =>
      value.channelIndex === index && typeof value === 'object'
    )?.[1];
  },
  ...
  getBestMatchCode(name) {
    ...
    if (uniqueCodes.length > 1) {
      console.error(`Ambiguous channel name: ${name} matches multiple channel codes: ${result.join(', ')}`);
      return null;
    }
  ...
  getAllDistinctCodes() {
    ...
    const sortedChannels = validChannels.sort((a, b) =>
      a.measurementOrder - b.measurementOrder
    );
