...
- `sub-search.js` : utilise `options.globalTheoreticalMax` (passé depuis `flow.js`)
...
### `globalTheoreticalMax` partagé

Le `globalTheoreticalMax` est calculé **une fois** au début (à partir des subs
originaux non paramétrés) et passé à tous les subs via `options`. Cela
garantit que tous les subs sont scorés contre la **même référence stable**.

> **Piège** : calculer un `theo` per-sub (`[sub + previousSum]`) crée une cible
> mouvante. La somme précédente change à chaque étape (avec les params
> appliqués), donc le `theo` change aussi. Une solution bonne localement peut
> dégrader l'efficacité globale.
...
### ⚠️ Ne pas calculer de theo per-sub

Un `theo` calculé comme `[sub + previousSum]` change à chaque étape (car
`previousSum` évolue). Cela crée une cible mouvante et des scores non
comparables entre subs. **Utiliser le `globalTheoreticalMax` partagé.**
...
| ------------- | -------- | --------------------- | --------------------- |
| data.test     | 48.36%   | **55.33%** (+7)       | **80.63%** (+32)      |
| data.bug.test | 56.55%   | **68.21%** (+12)      | **69.99%** (+13)      |
