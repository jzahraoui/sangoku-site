...
describe('runPhaseMatchFilter', () => {
  // Bank fixture: slot 1 = joint per-sub filter (non-auto), slot 20 = all-pass
  // reservation, slots 21-22 manual — the shared-EQ reservations of a
  // projection. Everything else is a free auto slot.
  function bankWithReservations(overrides = []) {
    const bank = createEmptyFilters();
    bank[0] = { index: 1, type: 'PK', enabled: true, isAuto: false, frequency: 45, q: 5, gaindB: -4 };
    bank[19] = { index: 20, type: 'All pass', enabled: true, isAuto: false, frequency: 90, q: 0.5 };
    bank[20].isAuto = false;
    bank[21].isAuto = false;
    for (const filter of overrides) {
      bank[filter.index - 1] = filter;
    }
    return bank;
  }

  function phaseMatchHarness({ computed, bank }) {
    const rew = {
      getFrequencyResponse: vi.fn().mockResolvedValue({ freqs: [20], magnitude: [75] }),
      getTargetResponse: vi.fn().mockResolvedValue({ freqs: [20], magnitude: [75] }),
      getFilters: vi.fn().mockResolvedValue(bank),
    ...
    const calculator = {
      ...
      filterSet: { getActiveFilters: () => computed },
    };
    const ctx = {
      session: {},
      smoothingMethod: '1/6',
      createCalculator: vi.fn().mockReturnValue(calculator),
    };
    return { rew, ctx };
  }

  it('drops filters below minFilterGain and packs around the reserved slots', async () => {
    const { rew, ctx } = phaseMatchHarness({
      bank: bankWithReservations(),
      computed: [
        { filterType: 'PEAKING', fc: 40, Q: 4, gain: -3 },
        { filterType: 'PEAKING', fc: 60, Q: 5, gain: -0.2 },
      ],
    });

    await ops.runPhaseMatchFilter(rew, record({ sampleRate: 48000 }), ctx, 20, 200);

    const { filters } = rew.postFilters.mock.calls[0][1];
  ...
  it('clears the free slots when every computed filter is below the threshold', async () => {
    const leftover = {
      index: 5,
      type: 'PK',
      enabled: true,
    ...
    const { rew, ctx } = phaseMatchHarness({
      bank: bankWithReservations([leftover]),
      computed: [{ filterType: 'PEAKING', fc: 60, Q: 5, gain: -0.2 }],
    });

    await ops.runPhaseMatchFilter(rew, record({ sampleRate: 48000 }), ctx, 20, 200);

    const { filters } = rew.postFilters.mock.calls[0][1];
    expect(filters).toEqual([{ index: 5, type: 'None', enabled: true, isAuto: true }]);
  });

  it('throws when the optimizer produced no filter at all', async () => {
    const { rew, ctx } = phaseMatchHarness({
      bank: bankWithReservations(),
      computed: [],
    });
    ...
    await expect(
      ops.runPhaseMatchFilter(rew, record({ sampleRate: 48000 }), ctx, 20, 200),
    ).rejects.toThrow('No filters generated by optimizer');
  });

  it('recale la cible quand une fenêtre MTW est active (compensation de référentiel)', async () => {
    const windows = { leftWindowType: 'Rectangular', addFDW: false, addMTW: true };
    const freqs = [50, 100, 200];
    const working = { freqs, magnitude: [73, 73, 73] };
    const raw = { freqs, magnitude: [75, 75, 75] };
    ...
    rew.getFrequencyResponse = vi
      .fn()
      .mockResolvedValueOnce(working) // courbe de travail (fenêtrée)
      .mockResolvedValueOnce(raw); // relecture fenêtres désactivées
    rew.getTargetResponse = vi.fn().mockResolvedValue({ freqs, magnitude: [75, 75, 75] });
    rew.getIRWindows = vi.fn().mockResolvedValue(windows);
    rew.setIRWindows = vi.fn().mockResolvedValue(true);

    await ops.runPhaseMatchFilter(rew, record({ sampleRate: 48000 }), ctx, 20, 200);

    // Fenêtres désactivées pour lire la brute, puis restaurées verbatim
    expect(rew.setIRWindows).toHaveBeenNthCalledWith(1, expect.anything(), {
      ...windows,
      addMTW: false,
      addFDW: false,
    });
    expect(rew.setIRWindows).toHaveBeenNthCalledWith(2, expect.anything(), windows);

    // Cible abaissée de l'offset mesuré (brute − travail = +2 dB)
  ...
  it('ne compense pas quand aucune fenêtre MTW/FDW n’est active', async () => {
    ...
    rew.getTargetResponse = vi.fn().mockResolvedValue({ freqs, magnitude: [75, 75] });
    rew.getIRWindows = vi.fn().mockResolvedValue({ addMTW: false, addFDW: false });
    rew.setIRWindows = vi.fn();
    ...
    const calculator = ctx.createCalculator.mock.results[0].value;
    const targetPassed = calculator.calculate.mock.calls[0][1];
    expect(targetPassed.magnitude).toEqual([75, 75]);
  });
});
