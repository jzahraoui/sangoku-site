...
class REWMeasurements {
  ...
  async getFrequencyResponse(id, options = {}) {
    ...
    return {
      ...data,
      magnitudeArray: this.decodeBase64ToFloat32(data.magnitude),
      phaseArray: data.phase ? this.decodeBase64ToFloat32(data.phase) : null,
    };
  }
  ...
  async getGroupDelay(id, options = {}) {
    ...
    return {
      ...data,
      delayArray: this.decodeBase64ToFloat32(data.magnitude),
    };
  }
  ...
  async getImpulseResponse(id, options = {}) {
    ...
    return {
      ...data,
      dataArray: this.decodeBase64ToFloat32(data.data),
    };
  }
   ...
   * A + B
   */
  async arithmeticAPlusB(measurementUUIDs) {
    return this.arithmetic(measurementUUIDs, 'A + B');
  }

  /**
   * A - B
   */
  async arithmeticAMinusB(measurementUUIDs) {
    return this.arithmetic(measurementUUIDs, 'A - B');
  }
   ...
   * A * B
   */
  async arithmeticATimesB(measurementUUIDs) {
    return this.arithmetic(measurementUUIDs, 'A * B');
  }
  ...
  async arithmeticOneDividedByA(
    ...
    return this.arithmetic(measurementUUIDs, '1 / A', {
      maxGain,
      lowerLimit,
      upperLimit,
      targetLevel,
  ...
  async arithmeticOneDividedByB(
    ...
    return this.arithmetic(measurementUUIDs, '1 / B', {
      maxGain,
      lowerLimit,
      upperLimit,
      targetLevel,
   ...
   * A * B conjugate
   */
  async arithmeticATimesBConjugate(measurementUUIDs) {
    return this.arithmetic(measurementUUIDs, 'A * B conjugate');
  }
   ...
   * (A + B) / 2
   */
  async arithmeticAPlusBDividedBy2(measurementUUIDs) {
    return this.arithmetic(measurementUUIDs, '(A + B) / 2');
  }

  /**
   * Merge B to A
   */
  async arithmeticMergeBToA(measurementUUIDs, frequencyHz = 200, blend = false) {
    return this.arithmetic(measurementUUIDs, 'Merge B to A', {
      frequencyHz: String(frequencyHz),
      blend,
    });
  }
   ...
   * Invert A phase
   */
  async arithmeticInvertAPhase(measurementUUIDs) {
    return this.arithmetic(measurementUUIDs, 'Invert A phase');
  }

  /**
   * Invert B phase
   */
  async arithmeticInvertBPhase(measurementUUIDs) {
    return this.arithmetic(measurementUUIDs, 'Invert B phase');
  }
