...
while getopts "dp:r:h" opt; do
  case $opt in
    ...
    h) echo "Usage: $0 [-d] [-p profile] [-r region]"
       echo "  -d: Dry run mode"
...
invalidate_cache() {
  ...
  local distribution_id=$(aws cloudformation list-exports \
  ...
  if [ "$DRYRUN" = 0 ]; then
    ...
    local invalidation_id=$(echo "$invalidation_output" | jq -r '.Invalidation.Id')
