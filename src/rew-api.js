...
export default class RewApi {
  ...
  async initializeAPI() {
    ...
    // Active le mode blocking pour simplifier le scripting
    ...
    // Vérifie que l'audio est prêt (règle critique)
    await this.fetchWithRetry('/audio/status', 'GET', null, 10, {
      ready: true,
    });
  }
  ...
  async request(endpoint, method = 'GET', body = null) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const parsedMessage = RewApi.safeParseJSON(error.message);
        const errorMessage =
  ...
  async fetchWithRetry(
    ...
    try {
      ...
      const processID = this.extractProcessID(data, endpoint);
  ...
  extractProcessID(data, url) {
    ...
    if (typeof data === 'string') {
      const messageMatch = extractMatch(data);
    ...
    if (url !== '/measurements/process-result') {
      const messageMatch = extractMatch(data.processName);
      if (messageMatch) return messageMatch;
    }
  ...
  decodeBase64ToFloat32(base64String, isLittleEndian = false) {
